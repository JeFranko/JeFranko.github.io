...
\email{\href{mailto:jfranklin1185@gmail.com}{jfranklin1185@gmail.com}}%; jesse.franklin@uvm.edu}
	...
	\begin{abstract}
		...
		%The best known result for general level $N$ is \cite[Proposition $4.16$]{Armana-thesis} which demonstrates that for any level, $M^2_{2,1}(\Gamma_0(N)),$ the double cusp forms of weight $2$ and type $1$ are (analytic) holomorphic differentials on a (rigid analytic) Drinfeld modular curve $\Gamma_0(N)\setminus(\Omega\cup \bbP^1(\bbF_q[T])).$\\
		...
		\begin{proof}
			...
			%Furthermore, $\Lambda$ is similar to $(\omega_1/\omega_2)A+A,$ where $z=\omega_1/\omega_2\in \Omega.$ So the exponential functions $e_{\bar{\pi}\alpha A}(\bar{\pi}\alpha z)$ and $e_A(z)$ for $z\in \Omega$ differ by a factor of $\bar{\pi}\alpha^{-1}.$
		\end{proof}
		...
		\begin{proof}
			...
			%\cite[Theorem $7.4$]{Porta-Yu-Higher-analytic-stacks-GAGA}
		...
		\begin{proof}
			...
			%The left cosets of $\bbF_q^{\times}$ in $\Gamma_e$ have representatives 
			...
			%\[\det\left(\frac{\gamma^j}{\alpha}\right)=\frac{\det\gamma^j}{\alpha^2}\]
			%is a quotient of squares so is a square. 
			...
			%\[\det(\gamma^j/\alpha_i)=\frac{\alpha'\det\gamma_2}{\alpha_i^2},\] 
...
To apply Theorem \ref{thm: forms to differentials} we need the following geometric invariants of %$\sX(\Gamma_1(\alpha T+\beta))$ and 
